...
const Time = styled.span`
	${(props: TimeProps) => props.flash && `animation: ${flash} linear 500ms alternate infinite`}
`;
...
const component = ({ controller, timer, timeLimit }: TimerProps) => {
	...
	return (
		<MatchTimer>
			{timer > secondsLimit * 60 && scoresEqual(controller.getCurrentState()) && <Overtime>overtime</Overtime>}
			<Time flash={timer <= secondsLimit && timer > secondsLimit - 30}>{time}</Time>
		</MatchTimer>
	);
};

const areEqual = (prevProps: TimerProps, nextProps: TimerProps) =>
	Math.floor(nextProps.timer) === Math.floor(prevProps.timer);
