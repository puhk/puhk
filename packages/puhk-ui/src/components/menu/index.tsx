...
export default class Menu extends React.Component<StateControllerProps> {
	...
	changeScoreLimit = (event: React.SyntheticEvent<HTMLInputElement>) => {
		...
		controller.addEvent(new Events.ChangeScoreLimit(gameState.frame, controller.getMe().id, { limit }));
	};
	...
	changeTimeLimit = (event: React.SyntheticEvent<HTMLInputElement>) => {
		...
		controller.addEvent(new Events.ChangeTimeLimit(gameState.frame, controller.getMe().id, { limit }));
	};
	...
	setRoomName = () => {
		...
		controller.addEvent(
			new Events.ChangeRoomName(gameState.frame, controller.getMe().id, { name: this.state.roomName })
		);
	};
