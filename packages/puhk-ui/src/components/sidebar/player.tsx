...
const toggleAdmin = (controller: NetworkController, player: Entities.Player) => {
	...
	controller.addEvent(new Events.PlayerAdmin(controller.getCurrentState().frame, controller.getMe().id, data));
};
...
export default React.memo(({ controller, player }: PlayerProps) => (
	<div>
		...
		<OptionsMenu id={`player-menu-${player.clientId}`}>
			<MenuItem onClick={() => toggleAdmin(controller, player)}>{`${player.admin ? 'Remove' : 'Make'} admin`}</MenuItem>
		</OptionsMenu>
	</div>
));
