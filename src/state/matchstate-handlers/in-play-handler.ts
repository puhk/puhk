...
const mapGoalsScoredToTeamScores = (goalsScored: GoalScored[], state: State): [string, number][] => {
    return goalsScored.map(goalScored => state.stadium.getTeam(goalScored.goal.data.teamScored))
...
export default (state: State, previousState: State) => {
    const scores = mapGoalsScoredToTeamScores(
        calculateGoalsScored(state, previousState),
        state
    );
